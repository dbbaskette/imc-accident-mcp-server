...
JAR_FILE="target/imc-accident-mcp-server-0.0.1-SNAPSHOT.jar"
PID_FILE_STDIO="/tmp/imc-accident-mcp-server-stdio.pid"
PID_FILE_SSE="/tmp/imc-accident-mcp-server-sse.pid"
PID_FILE_STDIO_TEST="/tmp/imc-accident-mcp-server-stdio-test.pid"
PID_FILE_SSE_TEST="/tmp/imc-accident-mcp-server-sse-test.pid"
LOG_FILE_STDIO="/tmp/imc-accident-mcp-server-stdio.log"
LOG_FILE_SSE="/tmp/imc-accident-mcp-server-sse.log"
LOG_FILE_STDIO_TEST="/tmp/imc-accident-mcp-server-stdio-test.log"
LOG_FILE_SSE_TEST="/tmp/imc-accident-mcp-server-sse-test.log"
...
start_server() {
    ...
    echo -e "${BLUE}${ROCKET} Starting IMC Accident MCP server in $TRANSPORT_MODE $profile_desc mode...${NC}"
    ...
    if kill -0 "$server_pid" 2>/dev/null; then
        echo -e "${GREEN}${CHECK_MARK} IMC Accident MCP server started successfully (PID: $server_pid)${NC}"
...
# Print header
echo -e "${PURPLE}╔══════════════════════════════════════════╗${NC}"
echo -e "${PURPLE}║      IMC Accident MCP Server Manager     ║${NC}"
echo -e "${PURPLE}╚══════════════════════════════════════════╝${NC}"
echo ""
