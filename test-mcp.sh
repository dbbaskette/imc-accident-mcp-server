...
kill_existing_servers() {
    ...
    local existing_pids=$(ps aux | grep "imc-accident-mcp-server-.*\.jar" | grep -v grep | awk '{print $2}')
    
    if [ -n "$existing_pids" ]; then
        echo -e "${YELLOW}Found existing IMC Accident MCP server processes:${NC}"
        ...
        local remaining_pids=$(ps aux | grep "imc-accident-mcp-server-.*\.jar" | grep -v grep | awk '{print $2}')
        ...
        echo -e "${GREEN}✓${NC} Existing IMC Accident MCP server processes terminated"
    else
        echo -e "${GREEN}✓${NC} No existing IMC Accident MCP server processes found"
    fi
...
test_stdio_tools() {
    ...
    echo -e '    "mcp-server": {'
    echo -e '      "command": "java",'
    echo -e '      "args": ["-Dspring.profiles.active=stdio-test", "-jar", "'$(pwd)'/target/imc-accident-mcp-server-0.0.1-SNAPSHOT.jar"]'
    echo -e '    }'
...
# Print header
echo -e "${PURPLE}╔══════════════════════════════════════════╗${NC}"
echo -e "${PURPLE}║      IMC Accident MCP Server Tester      ║${NC}"
echo -e "${PURPLE}╚══════════════════════════════════════════╝${NC}"
echo ""
...
if [ "$MODE" = "stdio" ]; then
    ...
    echo -e "${BLUE}${TEST_TUBE} IMC Accident MCP Server STDIO Mode Configuration${NC}"
    ...
    echo -e "${BLUE}${TEST_TUBE} IMC Accident MCP Server SSE Mode Configuration${NC}"
